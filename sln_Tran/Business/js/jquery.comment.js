(function($){
	function crateCommentInfo(obj){
		...
		//<i class='glyphicon glyphicon-globe'>   地球小图标		style="float: right;position: absolute;"  
		//var el = "<div class='comment-info'><header><img src='"+obj.img+"'></header><div class='comment-right'><h6>"+obj.replyName+"</h6>"
		//+"<div class='comment-content-header'><span><i class='glyphicon glyphicon-time'></i>发表时间 : "+obj.time+"</span>";
	
	//测试发表时间往右移
		var el = "<div class='comment-info'><header><img src='"+obj.img+"'></header><div class='comment-right'><h6>"+obj.replyName+"<small><i class='glyphicon glyphicon-time'></i>发表时间 : "+obj.Ctime+"</small></h6>"
		+"<div class='comment-content-header'>";

		//&& obj.browse != ""
		if(typeof(obj.address) != "undefined" ){
		//	el =el+"<span><i class='glyphicon glyphicon-map-marker'></i>类别 : "+obj.address+"</span>";
		}
		...
		if(typeof(obj.osname) != "undefined" && obj.osname != ""){
			el =el+"<span><i class='glyphicon glyphicon-map-marker'></i>类别 : "+obj.address+"    </span>";
			el =el+"<span><i class='glyphicon glyphicon-pushpin'></i> 任务状态 : "+obj.osname+"</span>";
		}
		
		if(typeof(obj.browse) != "undefined" && obj.browse != ""){
			el = el + "<span><i class='glyphicon glyphicon-time'></i> 完成时间 : "+obj.browse+"</span>";
		}
		//回复的按钮
...
var UserName=decodeURIComponent(getQueryVariable("UserName"))
	...
	$.fn.addCommentList=function(options){
		...
		if(option.data.length > 0){
			...
			$(this).append(totalString).find(".reply-btn").click(function(){   //回复按钮
			...
			$(".reply-list-btn").click(function(){   //回复评论  的回复按钮
				if($(this).parent().parent().find(".replybox").length > 0){
					$(".replybox").remove();
				}else{
					$(".replybox").remove();
